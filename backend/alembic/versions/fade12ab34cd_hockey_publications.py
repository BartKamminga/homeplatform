...
def upgrade():
    ...
    if "hockey_publications" not in existing:
        op.create_table(
            "hockey_publications",
    ...
    op.create_index("ix_hockey_publications_group_id", "hockey_publications", ["group_id"], if_not_exists=True)

    if "hockey_publication_tags" not in existing:
        op.create_table(
    ...
    if "hockey_publication_comps" not in existing:
        op.create_table(
    ...
    op.create_index("ix_hockey_publication_comps_publication_id", "hockey_publication_comps", ["publication_id"], if_not_exists=True)

    if "hockey_publication_comp_tags" not in existing:
        op.create_table(
    ...
    op.create_index("ix_hockey_publication_comp_tags_comp_link_id", "hockey_publication_comp_tags", ["comp_link_id"], if_not_exists=True)

    # Data migreren — alleen als brontabellen bestaan en doel leeg is
    if "tournix_tournaments" in existing:
        count = conn.execute(text("SELECT COUNT(*) FROM hockey_publications")).scalar()
        if count == 0:
            op.execute(sa.text(
    ...
    if "tournix_fase_tags" in existing:
        count = conn.execute(text("SELECT COUNT(*) FROM hockey_publication_tags")).scalar()
        if count == 0:
            op.execute(sa.text(
    ...
    if "tournix_tournament_competitions" in existing:
        count = conn.execute(text("SELECT COUNT(*) FROM hockey_publication_comps")).scalar()
        if count == 0:
            op.execute(sa.text(
    ...
    if "tournix_competition_fase_tags" in existing:
        count = conn.execute(text("SELECT COUNT(*) FROM hockey_publication_comp_tags")).scalar()
        if count == 0:
            op.execute(sa.text(
