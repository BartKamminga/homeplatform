...
export default function Sites() {
  ...
  const [form, setForm]       = useState({ name: '', slug: '', module: '' });
  ...
  const columns = [
    ...
    { key: 'allowed_groups', label: 'Toegang', render: v => v.length
    ...
    <AdminLayout>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: 'var(--font-size-sm)' }}>{sites.length} sites geregistreerd</p>
        </div>
        <button
      ...
      {showNew && (
        <Modal title="Nieuwe site registreren" onClose={() => setShowNew(false)}>
          {[['name','Naam'],['slug','Slug'],['module','Module naam']].map(([field, label]) => (
